...
ANGLE_STEP  = 0.1 #that means Poly-YOLO will detect 360/15=24 vertices per polygon at max
...
def yolo_body(inputs, num_boxes_per_level, num_classes):
    ...
    # downsample
    x_8 = DarknetConv2D_BN_Mish(base * 32, (3, 3), (2, 2))(all)
    x_16 = DarknetConv2D_BN_Mish(base * 32, (3, 3), (2, 2))(x_8)
    x_32 = DarknetConv2D_BN_Mish(base * 32, (3, 3), (2, 2))(x_16)

    # print("x.shape:", x.shape)
    print()
    print("num_classes:", num_classes)
    print("NUM_ANGLES:", NUM_ANGLES)
    print("num_boxes_per_level:", num_boxes_per_level)
    print()
    all = compose(
        DarknetConv2D_BN_Mish(num_filters * 2, (3, 3)),

        DarknetConv2D(num_boxes_per_level * (num_classes + 5 + NUM_ANGLES), (1, 1)))(x_32)
    print("all.shape:", all.shape)
    return Model(inputs, all)



def yolo_head(feats, num_boxes_per_level, num_classes, input_shape, calc_loss=False):
    ...
    num_boxes_per_level = num_boxes_per_level
    ...
    feats = tf.reshape(
        feats, [-1, grid_shape[0], grid_shape[1], num_boxes_per_level, num_classes + 5 + NUM_ANGLES], name='yolo_head/reshape/feats')
    ...
    polygons_dist = K.sigmoid(feats[..., 5 + num_classes:num_classes + 5 + NUM_ANGLES])
    ...
    # polygons_dist = polygons_dist/ diagonal
...
def yolo_boxes_and_scores(feats, num_boxes_per_level, num_classes, input_shape, image_shape):
    '''Process Conv layer output'''
    box_xy, box_wh, box_confidence, box_class_probs, polygons_dist = yolo_head(feats, num_boxes_per_level, num_classes, input_shape)
...
def yolo_eval(yolo_outputs,
    ...
    for l in range(1):
        _boxes, _box_scores, _polygons = yolo_boxes_and_scores(yolo_outputs , num_boxes_per_level, num_classes, input_shape,
                                                               image_shape)
        boxes.append(_boxes)
...
def my_preprocess_true_boxes_NPinterp(true_boxes, input_shape, num_boxes_per_level, num_classes):
    '''Preprocess true boxes to training input format

    Parameters
    ----------
    true_boxes: array, shape=(m, T, 5+69)  (4, 80, 3604) # batch size , total boxes, total true values
        Absolute x_min, y_min, x_max, y_max, class_id relative to input_shape
    input_shape: array-like, hw, multiples of 32
    anchors: array, shape=(N, 2), wh
    num_classes: integer
    ...
    m = true_boxes.shape[0]
    grid_shapes = [input_shape // {0: grid_size_multiplier}[l] for l in range(1)]
    y_true = [np.zeros((m, grid_shapes[l][0], grid_shapes[l][1], num_boxes_per_level, 5 + num_classes + NUM_ANGLES),
                       dtype='float32') for l in range(1)]
    ...
    for b in range(m):
        ...
        wh = boxes_wh[b, valid_mask[b]]
        # print("wh", wh)
        # print("true_boxes[b, valid_mask[b]]",true_boxes[b, valid_mask[b]].shape)
        if len(wh) == 0: continue

        for bi in range(true_boxes[b, valid_mask[b]].shape[0]):
            temp_box = true_boxes[b, valid_mask[b]][bi]
            # print("temp_box:", temp_box)
            i = np.floor(temp_box[0] * grid_shapes[0][1]).astype('int32')  # for each image b , and each box t
            j = np.floor(temp_box[1] * grid_shapes[0][0]).astype('int32')
            # print("i:", i)
            # print("j:", j)


            assert y_true[0][b, j, i, bi % num_boxes_per_level, 4] ==0,"num of boxes per level is not enough"

            c = temp_box[4].astype('int32')  # class number e.g 10 classses = [0 ,1, 2,3,4,...9], in our case ,one class only , c=0

            y_true[0][b, j, i, bi % num_boxes_per_level, 0:4] = temp_box[0:4] # set as raw  xmin, ymin , xmax ymax , predict the correponding position 9 boxes to be positive
            y_true[0][b, j, i, bi % num_boxes_per_level, 4] = 1 # F/B label
            y_true[0][b, j, i, bi % num_boxes_per_level, 5 + c] = 1  # class label as 1 as one-hot for class score. e.g. if 3 class, c =2 , then label in [5: num_classes] into  [0, 0 ,1]
            y_true[0][b, j, i, bi % num_boxes_per_level, 5 + num_classes:5 + num_classes + NUM_ANGLES] = temp_box[5: 5 + NUM_ANGLES]

    return y_true  # for shape[batch, gridH,gridW, num_boxes, ....]
...
def yolo_loss(args, num_boxes_per_level, num_classes, ignore_thresh=.4):
    ...
    for layer in range(num_layers):
        # preds -------------------------------------------------------->
        pred_xy, pred_wh, pred_confidence, pred_class_probs, pred_polygons_dist= yolo_head(yolo_outputs[layer], num_boxes_per_level, num_classes,
        ...
        raw_true_xy = y_true[layer][..., :2]
        raw_true_wh = y_true[layer][..., 2:4]
        # raw_true_wh = K.log(y_true[layer][..., 2:4]  * input_shape[::-1] + K.epsilon()) # to avoid log (0)
        ...
        # raw_true_polygon_dist = K.log(raw_true_polygon_distnace * diagonal + K.epsilon())  # the deviation of distances + epsiolon # to avoid log (0)
        raw_true_polygon_dist = K.switch(object_mask, raw_true_polygon_distnace, K.zeros_like(raw_true_polygon_distnace))  # only positives
        ...
        xy_loss = object_mask * box_loss_scale * K.binary_crossentropy(raw_true_xy, pred_xy, from_logits=False)
        wh_loss = object_mask * box_loss_scale * 0.5 * K.square(raw_true_wh - pred_wh)

        # (1 - object_mask) for background response
        confidence_loss = object_mask * K.binary_crossentropy(object_mask, pred_confidence, from_logits=False) + (1 - object_mask) * K.binary_crossentropy(object_mask, pred_confidence,
                                                                                                                                                             from_logits=False) * ignore_mask # ignore_mask for negatives
        class_loss = object_mask * K.binary_crossentropy(true_class_probs, pred_class_probs, from_logits=False)
        polygon_loss_dist_L2 = object_mask * box_loss_scale * 0.5 * K.square(raw_true_polygon_dist - pred_polygons_dist)
        ...
        # polygon_dist_loss = K.sum(polygon_loss_dist_L2) / mf
        # #
        # polar_diou_loss = K.sum(object_mask * box_loss_scale * (1 - polar_diou(pred_box, y_true[layer][..., 0:4],
        #                                                                        raw_pred[..., 5 + num_classes:5 + num_classes + NUM_ANGLES],  raw_true_polygon_dist))) / mf
        ...
        print("finished losses")
        xy_loss = K.sum(xy_loss)  # get the mean loss for each image
        ...
        #
        Polar_diou = polar_diou(pred_box, y_true[layer][..., 0:4], pred_polygons_dist,raw_true_polygon_distnace)
...
class YOLO(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.num_boxes_per_level = self.num_boxes_per_level
    ...
    def generate(self):
        ...
        num_boxes_per_level = self.num_boxes_per_level
        num_classes = len(self.class_names)
        try:
            self.yolo_model = load_model(model_path, compile=False)
        except:
            self.yolo_model = yolo_body(Input(shape=(256, 256, 3)), num_boxes_per_level, num_classes)
            self.yolo_model.load_weights(self.model_path)  # make sure model, anchors and classes match
        else:
            # novy output
            assert self.yolo_model.layers[-1].output_shape[-1] == \
                   num_boxes_per_level / len(self.yolo_model.output) * (num_classes + 5 + NUM_ANGLES), \
        ...
        boxes, scores, classes, polygons = yolo_eval(self.yolo_model.output, self.num_boxes_per_level,
...
def my_Gnearator(images_list, masks_list, batch_size, input_shape, num_boxes_per_level, num_classes, train_flag):
    """
    :param images_list:
    :param masks_list:
    :param batch_size:
    ...
    while True:
        ...
        y_true = my_preprocess_true_boxes_NPinterp(box_batch, input_shape, num_boxes_per_level, num_classes)
...
if __name__ == "__main__":
    ...
    def _main():
        project_name = 'P_Part1_BK_Exp13_MyXceptionAnchorFree_SimulatedData_FA_AS14_{}'.format(model_index)
        ...
        if phase == 1:
            model = create_model(input_shape, num_boxes_per_level, num_classes, load_pretrained=False)
        else:
            model = create_model(input_shape, num_boxes_per_level, num_classes, load_pretrained=True, weights_path=log_dir+'poly_yolo.h5')
        ...
        train_Gen = my_Gnearator(train_input_paths, train_mask_paths, batch_size=4, input_shape=[256, 256],
                                   num_boxes_per_level= num_boxes_per_level, num_classes=num_classes,
                                     train_flag="Train")
    ...
    def create_model(input_shape, num_boxes_per_level, num_classes, load_pretrained=True, freeze_body=2,
        ...
        y_true = Input(shape=(h // grid_size_multiplier, w // grid_size_multiplier, num_boxes_per_level, num_classes + 5 + NUM_ANGLES))
        ...
        print('Create Poly-YOLO model with {} num_boxes_per_level and {} classes.'.format(num_boxes_per_level, num_classes))
        ...
        xy_loss, wh_loss, confidence_loss, polar_diou_loss, class_loss, polygon_dist_loss = Lambda(yolo_loss, output_shape=(1,), name='yolo_loss',
                            arguments={'num_boxes_per_level': num_boxes_per_level, 'num_classes': num_classes, 'ignore_thresh': 0.5})(
