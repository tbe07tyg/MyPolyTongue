...
try:
    def infer_case(Saved_model_file_root, output_folder, inference_folder, class_path):
        ...
        for inferd_case_dir in infered_Top_cases:  # case level
            ...
            #             summary_for_plot.append([index, infered_sub_root_name, type, AP_5to95, AP_5,AP_75])
            ...
            #             # f.write(content)
            #
            # data_to_plot = DataFrame(summary_for_plot, columns=["x",'infered_sub_root', 'type',
            #                                                     "$\mathregular{mAP_{0.5_0.95}}$",
            ...
            map_range_data = data_to_plot.melt('x', var_name='$\mathregular{mAP_{IoU}}$',  value_name='mAP', value_vars=["$\mathregular{mAP_{0.5}}$","$\mathregular{mAP_{0.5_0.95}}$","$\mathregular{mAP_{0.75}}$"])
            tyep_data = data_to_plot.melt('type', var_name='$\mathregular{mAP_{IoU}}$', value_name='mAP',
                                          value_vars=["$\mathregular{mAP_{0.5}}$", "$\mathregular{mAP_{0.5_0.95}}$",
                                                      "$\mathregular{mAP_{0.75}}$"])
            ...
            axes = sns.lineplot(x="x",
                         y="mAP",
                         data=merged_new_data_plot,markers=True, hue= "$\mathregular{mAP_{IoU}}$", style="type", color=colors[case_count])
...
if __name__ == '__main__':
    ...
    Saved_model_file_root =  "E:\\MyWritings\\Tongue\\2020IEEE\\traindCodesAndLogs\\randomnessArranged"
    # Inference_scripts_root = "E:\\Projects\\MyPolyTongue\\TonguePlusData\\EXP_2_Mish"

    output_folder =  "E:\\MyWritings\\Tongue\\2020IEEE\\Inferenced_Results\\RandomNessAnalyze"
