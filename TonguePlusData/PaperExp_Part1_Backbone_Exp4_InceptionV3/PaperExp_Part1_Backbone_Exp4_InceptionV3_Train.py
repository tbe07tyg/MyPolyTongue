...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    base_model = InceptionV3(input_tensor=inputs, weights=None, include_top=False) # random initialization
    # extract features from each block end: ["add", "add_1", "add_10", "add_11"]
    tiny = base_model.get_layer('activation_4').output   # 64x64 : 1/4
    small = base_model.get_layer('mixed2').output  # 32x32 : 1/8
    medium = base_model.get_layer('mixed7').output  # 16x16 : 1/16
    big = base_model.get_layer('mixed10').output  # 8x8 : 1/32
...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    print("polygons_x:", polygons_x) # shape=(None, None, None, 9, 24) , distance predictions from vertix to center (px)? (px, py)

    dx = K.square(anchors_tensor[..., 0:1] / 2)
    dy = K.square(anchors_tensor[..., 1:2] / 2)
    d = K.cast(K.sqrt(dx + dy), K.dtype(polygons_x)) # anchors distance
    ...
    polygons_x = polygons_x * d / diagonal  # see one note
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5):
    ...
    for layer in range(num_layers):
        ...
        print("5 + num_classes + NUM_ANGLES3:", 5 + num_classes + NUM_ANGLES3)
        vertices_mask = y_true[layer][..., 5 + num_classes + 2:5 + num_classes + NUM_ANGLES3:3]  # problabilies for each section
        print("vertices_mask:", vertices_mask)
        true_class_probs = y_true[layer][..., 5:5 + num_classes]
        print("true_class_probs:", true_class_probs)
        # return yolo_head: grid, feats, box_xy, box_wh, polygons_confidence
        ...
        print("raw_true_polygon0:", raw_true_polygon0)
        ...
        raw_true_polygon_x = raw_true_polygon0[..., ::3]  # ground truth, px,
        raw_true_polygon_y = raw_true_polygon0[..., 1::3] # ground truth, py,

        dx = K.square(anchors[anchor_mask[layer]][..., 0:1] / 2)  # get gt anchor x
        dy = K.square(anchors[anchor_mask[layer]][..., 1:2] / 2)  # get gt anchor y
        d = K.cast(K.sqrt(dx + dy), K.dtype(raw_true_polygon_x))  #  get gt d

        diagonal = K.sqrt(K.pow(input_shape[::-1][0], 2) + K.pow(input_shape[::-1][1], 2)) # get diagnoal of feature maps
        raw_true_polygon_x = K.log(raw_true_polygon_x / d * diagonal)  # the deviation of distances
...
if __name__ == "__main__":
    ...
    def _main():
        project_name = 'PaperExp_Part1_Backbone_Exp4_InceptionV3_FullAug_AngleStep17_{}'.format(model_index)
